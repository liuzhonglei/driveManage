...
select id_in_teacher, count(if( status > 0,true,null)) suc_count, concat ( left (ifnull(count(if( status > 0,true,null))/count(id),0)*100,5),'%')  suc_rate from wp_student  group by id_in_teacher
;

CREATE VIEW wp_teacher_rank_all AS
select  T1.id, T1.name ,ifnull(T3.suc_count,0)  suc_count, ifnull(T3.suc_rate,'0%') suc_rate from wp_teacher T1
left join
wp_teacher_rank T3
on T1.id = T3.id_in_teacher
order by suc_count
;
