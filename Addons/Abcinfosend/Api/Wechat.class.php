...
class Wechat
{
	...
	*/
	 ...
	 * @return boolean/array
	 */
	public function sendall($group_id,$media_id,$msgtype='mpnews'){
		$senddata=array(
				'filter'=>array(
						'group_id'=>$group_id
					),
    ...
    public function sendTempMes($openid, $tempId, $data, $url=null){
        $senddata=array(
            'touser'=>$openid,
            'template_id'=>$tempId,
            'url'=>$url,
        ...
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin/message/template/send?access_token='.$this->access_token,self::json_encode($senddata));
        if ($result)
        {
            $json = json_decode($result,true);
            return $json;
